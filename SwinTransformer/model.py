...
def window_partition(x, window_size: int):
    """
    # 将 feature map 按照 window_size 划分成一个个没有重叠的 window
    :param x: [B, H, W, C]
...
def window_reverse(windows, window_size: int, H: int, W: int):
    """
    # 与 window_partition 相反,将一个个 window 还原成一个 feature map
    ...
    # windows.shape[0]对应 num_windows
...
class PatchEmbed(nn.Module):
    ...
    对应结构图中的 Patch Partition 和 Linear Embedding
    """
    def __init__(self, patch_size=4, in_c=3, embed_dim=96, norm_layer=None):
        """
        :param patch_size: 每个 patch的大小，即 down sample rate
        :param in_c: 图像深度
        :param embed_dim: 映射到指定深度
        :param norm_layer: 传入的 Layer_norm 方法
        """
        super().__init__()
...
# model1 = swin_tiny_patch4_window7_224()
# print(model1)
